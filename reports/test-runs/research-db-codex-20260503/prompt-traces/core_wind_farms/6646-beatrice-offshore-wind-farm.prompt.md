...
Authoritative regional source-of-record row metadata (core_wind_farms; cleaned EMODnet values are enrichment-only when matched):
...
- Primary row source: Crown Estate Scotland
- Geometry source: Crown Estate Scotland
- Source precedence policy: uk_crown_estate_scotland_wind_farms
...
- Current database winner candidate: 84 (Calculated from 84 linked EuroWindWakes turbine locations)
- EuroWindWakes calculated linked-turbine count: 84 (Calculated from 84 linked EuroWindWakes turbine locations)
- EMODnet turbine-count hint: 84
